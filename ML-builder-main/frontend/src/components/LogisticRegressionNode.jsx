...
const LogisticRegressionNode = ({ data, id }) => {
  ...
  useEffect(() => {
    const processInput = async () => {
      ...
      console.log('🎯 LogisticRegression: Checking inputData:', data?.inputData);
      console.log('🎯 LogisticRegression: Found parsedInput:', parsedInput);

      if (parsedInput && parsedInput.data && parsedInput.headers) {
        console.log('🎯 LogisticRegression: Processing new data', parsedInput);
        setInputData(parsedInput);
        setError(null);

        // Auto-suggest first text/categorical column as target if none selected
  ...
  const executeModel = async () => {
    ...
    try {
      console.log('🚀 Running Logistic Regression with config:', config);
      console.log('🔍 Input data:', inputData);

      // Use real ML service - pass the data array and target column in config
      const mlResults = await AdvancedMLService.performAdvancedLogisticRegression(inputData.data, {
      ...
      console.log('✅ Logistic Regression completed:', mlResults);
    } catch (error) {
      console.error('❌ Logistic Regression failed:', error);
  ...
  return (
    <div className={`custom-node ml-node logistic-regression-node ${status}`}>
      <div className="node-header">
        <div className="node-icon">🎯</div>
        <div className="node-title">Logistic Regression</div>
        {status !== 'idle' && (
          <div
      ...
      <div className="node-content">
        ...
        <button
          ...
          style={{ 
            ...
            padding: '6px', 
            fontSize: '11px', 
            backgroundColor: isRunning || !inputData || !config.targetColumn ? '#6b7280' : '#8b5cf6',
            color: 'white',
            border: 'none',
            borderRadius: '4px',
...
export default LogisticRegressionNode;
